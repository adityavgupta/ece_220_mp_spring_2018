...
*/
/*Intro Paragraph
  This program uses a tree to sort out through different shapes in order to make it less complicated.
  This is an application of circuitry, in order to make complex circuit easier to understand.
  This was done by having a binary tree, in which we traversed ans sliced the tree at differenet points.
  The first thing we did was check if a node was a leaf node i.e. it points to NULL.
  We check if a node is an internal node in the function is_internal_node
  The other functions implemented were : recut, rotate, swap_module, swap_topology to manipulate the tree.
*/
...
// to do this check if the left child of the pointer and the right child of the pointer are null.
...
// this function checks if the node is the internal node.
// if the pointer left and right child is not NULL, then it is an internal node
...
//  this function if the given node is in the subtrees
//  this is achieved using recursion
// base case: if a == b then Return
//            b->parent
// recursive case : pass a, b->parent as the arguments in the function
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  // declare a pointer if the type module_t and set it to a->modules
  // set a->module to b->module which swaps them
  // set the b->module to temp thus completing the swap
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  // if a is the left child set the left child to b
  // if a is the  right child then change the right child to b
  // if b is the left child set that left child to a
  // if b is the right child set that right child to a
  ...
  // declare a temp pointer of the type node_t
  // set temp to a->parent
  // set a->parent to b->parent and then b->parent to temp, thus completing the swap swap_topology
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // recursive call for ptr->left and ptr->right
  // if the ptr->cutline is not UNDEFINED_CUTLINE then use the pointer "nth" to get value of ptr->cutline_t
  // set the expression module indicated by pointer "nth" to null
  ...
  // If the module pointer exists in the expression unit,set the corresponding cutline type to"UNDEFINED_CUTLINE".
  // If the expression unit is a cutline type,assign NULL to the corresponding module pointer.
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* base = (node_t*)malloc(sizeof(node_t));
	...
	if(n != num_modules - 1){
		...
		base->parent = par;
		base->right = right;
		base->left = init_slicing_tree(base, n+1);    //recursively call the left sub-tree
		base->module = NULL;
		base->cutline = V;
		...
		base->left = NULL;
		base->right = NULL;
		base->module = &modules[n];
		base->cutline = UNDEFINED_CUTLINE;
